...
# ------------------------------------frame 
        ...
        #serch-------------
            ...
            sr=customtkinter.CTkButton(master=my_w,text_color="black" ,hover_color="darkslategrey",text="بحث", text_font=("times new roman",15),bg_color="white",fg_color="cornsilk",cursor="hand2", command=serch)
...
.date{
    ...
    font-style: inherit;
            ...
            # btn4=customtkinter.CTkButton(,text_font=(20), text="Imprimer طباعة",command=imp, width=50, height=40, compound="right",fg_color="green", hover_color="#C77C78")
...
#reloadfunction
        ...
        btn1=customtkinter.CTkButton(master=self.root,command=select,text="vider bus",text_font=(15),width=scr_width/8, height=scr_height/18,text_color="black", compound="right",hover_color="dimgray",fg_color="darkkhaki")
...
#                 for dt in res:
#                     trv.insert("", 'end',iid=dt['id'], text=dt['id'],
#                     values =(dt['id'],dt['nom'],dt['numero'],dt['telephone'],dt['direction'],dt['prix'],dt['date'],dt['payer'],dt['emploiyer'],dt['imprimer'],dt['bus_num'],dt['depart']))
...
#                 # r_set=cur.execute("SELECT * from client ")
#                 # res=cur.fetchall()
#                 q="SELECT * FROM client WHERE CURRENT_DATE()=date and bus_num=%s and depart=%s order by id desc LIMIT "+ str(offset) +","+str(limit)
#                 cur.execute(q,(self.bus_numero,self.depart))
#                 print(self.bus_numero)
#                 res=cur.fetchall()
#                 for dt in res:
...
#                 value="✖"
#                 op=simpledialog.askinteger("Input","أدخل عدد المسافرين للطباعة",parent=my_w)
#                 q="SELECT numero,telephone,payer,nom,direction  from client where CURRENT_DATE()=date  and bus_num=%s and depart=%s and not imprimer=%s order by (id) desc limit %s"
#                 cur.execute(q,(self.bus_numero,self.depart,value,op))
#                 res=cur.fetchall()
